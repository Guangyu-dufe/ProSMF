...
class RNDPredictor(nn.Module):
    ...
    def forward(self, text_feat, density):
        ...
        text_emb = text_emb.unsqueeze(1)  # [B, 1, hidden_dim]
        density_emb = self.density_proj(density).unsqueeze(1)  # [B, 1, hidden_dim]
        proto_emb = selected_proto.unsqueeze(1)  # [B, 1, hidden_dim]
        
        tokens = torch.cat([text_emb, density_emb, proto_emb], dim=1)  # [B, 3, hidden_dim]
        
        rnn_out, _ = self.rnn(tokens)  # [B, 3, hidden_dim*2]
