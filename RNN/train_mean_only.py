...
class MeanPredictor(nn.Module):
    def __init__(self, bert_dim=768, density_dim=1000, hidden_dim=512, n_heads=8, n_layers=4):
        super().__init__()
    ...
    def forward(self, text_feat, density):
        ...
        return self.output_head(final_out).squeeze(-1)
...
def main():
    ...
    train_cache = os.path.join(CACHE_DIR, "train_transformer_mean_features.pt")
    val_cache = os.path.join(CACHE_DIR, "val_transformer_mean_features.pt")
    test_cache = os.path.join(CACHE_DIR, "test_transformer_mean_features.pt")
    ...
    for epoch in range(1, epochs + 1):
        ...
        if val_metrics['loss'] < best_val_loss:
            ...
            torch.save(model.state_dict(), 'best_model_transformer_mean.pt')
            log_msg += " *"
            no_improve = 0
        else:
            no_improve += 1
    ...
    model.load_state_dict(torch.load('best_model_transformer_mean.pt'))
