...
class RNDPredictor(nn.Module):
    def __init__(self, bert_dim=768, density_dim=1000, hidden_dim=512, output_dim=8, n_proto=6):
        super().__init__()
    ...
    def forward(self, text_feat, density):
        ...
        fused = torch.cat([selected_proto, density_feat], dim=-1)
...
def main():
    ...
    for epoch in range(1, epochs + 1):
        ...
        if val_metrics['loss'] < best_val_loss:
            ...
            torch.save(model.state_dict(), 'best_model_metrics.pt')
            log_msg += " *"
            no_improve = 0
        else:
            no_improve += 1
    ...
    model.load_state_dict(torch.load('best_model_metrics.pt'))
