...
def reconstruct_density_from_quantiles(q_probs, q_vals, n_points=500):
    ...
    if q_probs[0] > eps:
        ...
        q_vals = np.concatenate([[q_vals[0] - abs(q_vals[1] - q_vals[0])], q_vals])
    if q_probs[-1] < 1 - eps:
        q_probs = np.concatenate([q_probs, [1 - eps]])
        q_vals = np.concatenate([q_vals, [q_vals[-1] + abs(q_vals[-1] - q_vals[-2])]])
    ...
    try:
        ...
        try:
            ...
            return q_vals, np.ones_like(q_vals) / len(q_vals)
    ...
    if np.any(~np.isfinite(density_mid)):
        density_mid = np.nan_to_num(density_mid, nan=0.0, posinf=0.0, neginf=0.0)
    ...
    if total > 1e-10:
        ...
        density = np.ones(n_points) / (returns[-1] - returns[0] + 1e-10)
...
def compute_metrics_from_density(returns, density):
    ...
    if np.any(~np.isfinite(returns)) or np.any(~np.isfinite(density)):
        return {k: 0.0 for k in ['mean', 'variance', 'skewness', 'excess_kurtosis', 
    ...
    def compute_var(alpha):
        try:
            if len(np.unique(cdf)) < 2:
                return mean
            f_inv = interpolate.interp1d(cdf, returns, kind='linear', 
                                         bounds_error=False, fill_value='extrapolate')
            val = float(f_inv(alpha))
            return val if np.isfinite(val) else mean
        except:
    ...
    def compute_es(alpha):
        ...
        mask = returns <= var
        if not np.any(mask):
            return returns[0] if len(returns) > 0 else mean
        ...
        if tail_prob > 1e-10:
            ...
            return es if np.isfinite(es) else var
        return var
...
class RNDQuantilePredictor(nn.Module):
    def __init__(self, bert_dim=768, density_dim=1000, hidden_dim=512, output_dim=N_QUANTILES):
        super().__init__()
