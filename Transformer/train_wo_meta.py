...
def setup_logger():
    ...
    log_file = os.path.join(LOG_DIR, f"train_wo_meta_{timestamp}.log")
    
    logger = logging.getLogger("RND_WO_META")
...
class RNDPredictorWoMeta(nn.Module):
    def __init__(self, bert_dim=768, density_dim=1000, hidden_dim=512, output_dim=8, n_proto=6, n_heads=8, n_layers=4):
        ...
        self.register_buffer('prototypes', torch.randn(n_proto, hidden_dim) * 0.02)
...
def main():
    ...
    logger.info("Variant: w/o Meta (fixed prototypes)")
    ...
    model = RNDPredictorWoMeta(bert_dim=768, density_dim=1000, hidden_dim=512, output_dim=8).to(device)
    ...
    for epoch in range(1, epochs + 1):
        ...
        if val_metrics['loss'] < best_val_loss:
            ...
            torch.save(model.state_dict(), 'best_model_wo_meta.pt')
            log_msg += " *"
            no_improve = 0
        else:
            no_improve += 1
    ...
    model.load_state_dict(torch.load('best_model_wo_meta.pt'))
