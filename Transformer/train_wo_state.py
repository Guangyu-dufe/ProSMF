...
def setup_logger():
    ...
    log_file = os.path.join(LOG_DIR, f"train_wo_state_{timestamp}.log")
    
    logger = logging.getLogger("RND_WO_STATE")
...
class RNDPredictorWoState(nn.Module):
    def __init__(self, bert_dim=768, hidden_dim=512, output_dim=8, n_proto=6, n_heads=8, n_layers=4):
        super().__init__()
    ...
    def forward(self, text_feat):
        ...
        tokens = torch.cat([text_emb, proto_emb], dim=1)
...
def train_epoch(model, loader, optimizer, device, epoch, epochs):
    ...
    for text_feat, _, targets in pbar:
        text_feat, targets = text_feat.to(device), targets.to(device)
        optimizer.zero_grad()
        pred = model(text_feat)
...
def main():
    ...
    logger.info("Variant: w/o State (news-only)")
    ...
    model = RNDPredictorWoState(bert_dim=768, hidden_dim=512, output_dim=8).to(device)
    ...
    for epoch in range(1, epochs + 1):
        ...
        if val_metrics['loss'] < best_val_loss:
            ...
            torch.save(model.state_dict(), 'best_model_wo_state.pt')
            log_msg += " *"
            no_improve = 0
        else:
            no_improve += 1
    ...
    model.load_state_dict(torch.load('best_model_wo_state.pt'))
